---
title: "Intrinsic and extrinsic drivers modulate the functional diversity of frugivorous birds interacting with individual trees within a population"
...
This document reproduces the fitting procedure of Bayesian models from the article **Intrinsic and extrinsic drivers modulate the functional diversity of frugivorous birds interacting with individual trees within a population**. It is organized into sections showing the data wrangling operations for preparing the data, the models in mathematical notation, Stan code, fitting procedure, models diagnostics, estimation of condition effect, and simulations of interventions.
...
The following code loads the data, transforms latitude/longitude coordinates to UTM for each individual, and generates the distance matrix among individuals. The additional code is used to format the `R` object holding the data set.
```{r}
...
# Mathematical notation of the models

We aimed to assess the effect of intrinsic and extrinsic drivers on bird functional diversity (FD) while accounting for the effect of species richness on FRic, FEve, and FReg. To do so, we used a Bayesian hierarchical approach to model the effect of predictors on species richness (S) and FD metrics as a set of hierarchical linear equations:

$$
\begin{aligned}
& S_i \sim Student\text{-}T(\nu = 7, ~\mu1_i, ~\sigma1) \\
& \mu1_i = \alpha_1 + \tau1_{individual~i} + \beta_1 \times crop~size_i + \\
& \beta_2 \times height_i + \beta_3 \times fruiting~duration_i + \\
&\beta_4 \times fruit~density_i + \beta_5 \times tree~cover_i + \\
&\beta_6 \times infrastructure~cover_i \\
\\
& FD_{metric~i} \sim Student\text{-}T(\nu = 7, ~\mu2_i, ~\sigma2) \\
& \mu2_i = \alpha_2 + \tau2_{individual~i} + \beta_7 \times S_i \\
& \beta_8 \times crop~size_i + \beta_9 \times height_i + \\
& \beta_{10} \times fruiting~duration_i + \\
&\beta_{11} \times fruit~density_i + \\
& \beta_{12} \times tree~cover_i + \\
&\beta_{13} \times infrastructure~cover_i \\
\end{aligned}
$$

We assign prior probabilities to the intercept (i.e., $\alpha$)

$$
\begin{aligned}
& \alpha_1 \sim Normal(0, 1) \\
& \alpha_2 \sim Normal(0, 1) \\
\end{aligned}
$$
Gaussian processes for accounting spatial correlation among individuals (i.e., $\tau$ parameter). We used a quadratic kernel (i.e., $k(d_i, ~d_j)$ in the formula below) to construct the covariance matrix ($K[i, j]$) and a non-centered parametrization through a Cholesky decomposition of the covariance matrix. 

$$
\begin{aligned}
& k(i, ~j) = \eta^2 exp(-\rho^2D_{ij}^2) + \delta_{ij} \times 0.01 \\
& K[i, j] = k(i, j) \\
& LK = K \times K^T \\
& \tau_{1, 2} = LK \times z_{\tau_{1, 2}} \\
& z_\tau \sim Normal(0, 1)\\
& \eta \sim exponential(4) \\
& \delta \sim exponential(1) \\
\end{aligned}
$$
Priors for slope parameters
$$
\begin{aligned}
&\beta_{1,~ ..., 13} \sim Normal(0, 0.25) \\
\end{aligned}
$$
...
*NOTE:* **The models for FRic, FDiv and FReg have the same structure. Therefore, the above mathematical version can be applied to all of them.**

# Model FRic

Formatting the data frame in a list object.
```{r}
...
```{r, fig.cap='Rhat values vs effective sampling size (ess). Rhat < 1.05 indicates that the Markov Chains converged to the same stationary distribution. ess must be at least 100 in order to be reliable'}
sum_FRic <- mod_FRic$summary()

mod_diagnostics(mod_FRic, sum_FRic)
```

```{r, fig.cap='Trace plots showing chain convergence per parameter'}
indx_pars <- 
...
```{r, fig.cap='Posterior predictive checks of the model describing species richness and FRic of the birds assemblage'}
...
```{r, fig.cap='Observed average and SD from species richness and FRic against the estimated by the model'}
par(mfrow = c(2, 2))
...
knitr::kable(
  sum_FRic[indx_pars, ], 
  digits = 2, 
  caption = 'Average, SD, confidence intervals and metrics of sampling diagnostics for the parameters estimating species richness and FRic of the birds assemblage'
)
```
...
#### Summary causal effect urbanization
...
knitr::kable(tribble(~ value,               ~estimation, 
...
#### Summary causal effect tree cover 
...
knitr::kable(
  tribble(~ value,               ~estimation, 
        ...
        'AVG diff total-conditional (%)', (mean(diff_forest_FRic)*100 / mean(cont_forest_FRic$est)-100)), digits = 2
)
...
```{r, fig.cap='Intrinsic (a) and extrinsic (b, c) drivers influencing the functional richness (FRic) and species richness (S) of birds foraging on an S. terebinthifolia population. Effects on S (a-i to c-i iii) imply indirect effects (i.e., X -> S -> FRic) on FRic, whereas effects on FRic (a-ii to c-ii) denote direct effects (a i-ii, b i-ii and c i-ii). The dots indicate observed values, the solid line is the average estimation, the inner bands include 95% CI of the estimation, and the outer bands show 95% CI of the prediction. Error bars and dots in a-iii, b-iii, and c-iii show 95% credibility intervals and average values of the causal effect of intervening factors to minimum and maximum values; the contrast shows the difference between such interventions. The total effect implies both direct and indirect effects of driver i on FRic. In contrast, the conditional effect denotes the direct effect of driver i on FRic while holding S on the minimum value.', fig.height= 15, fig.width=15}
...
```{r, fig.cap='Posterior predictive checks of the model describing species richness and FDiv of the birds assemblage'}
...
```{r, fig.cap='Observed average and SD from species richness and FDiv against the estimated by the model'}
par(mfrow = c(2, 2))
plot(density(apply(ppcheck_S, 1, mean)), xlab = 'AVG. S (estimated)', 
     main = '')
...
knitr::kable(
  sum_FDiv[indx_pars, ], 
  digits = 2, 
  caption = 'Average, SD, confidence intervals and metrics of sampling diagnostics for the parameters estimating species richness and FDiv of the birds assemblage'
)
```
...
knitr::kable(
  tribble(~ value,               ~estimation, 
        ...
        'AVG diff total-conditional (%)', mean(diff_height_FDiv)*100 / mean(cont_height_FDiv$est)), digits = 2
)
...
knitr::kable(
  tribble(~ value,               ~estimation, 
        ...
        'AVG diff total-conditional (%)', mean(diff_urban_FDiv)*100 / mean(cont_urban_FDiv$est)), digits = 2
)
```
...
knitr::kable(
  tribble(~ value,               ~estimation, 
        ...
        'AVG diff total-conditional (%)', mean(diff_crop_FDiv)*100 / mean(cont_crop_FDiv$est)), digits = 2
)
...
```{r, fig.cap='Intrinsic (a) and extrinsic (b, c) drivers influencing the functional divergence (FDiv) and species richness (S) of birds foraging on an S. terebinthifolia population. Effects on S (a-i to c-i iii) imply indirect effects (i.e., X -> S-> FDiv) on FDiv, whereas effects on FDiv (a-ii to c-ii) denote direct effects (a i-ii, b i-ii and c i-ii). The dots indicate observed values, the solid line is the average estimation, the inner bands include 95% CI of the estimation, and the outer bands show 95% CI of the prediction. Error bars and dots in a-iii, b-iii, and c-iii show 95% credibility intervals and average values of the causal effect of intervening factors to minimum and maximum values; the contrast shows the difference between such interventions. The total effect implies both direct and indirect effects of factor i on FDiv. In contrast, the conditional effect denotes the direct effect of factor i on FDiv while holding S on the minimum value.', fig.height= 15, fig.width=15}
...
```{r, fig.cap='Posterior predictive checks of the model describing species richness and FReg of the birds assemblage'}
...
```{r, fig.cap='Observed average and SD from species richness and FReg against the estimated by the model'}
par(mfrow = c(2, 2))
plot(density(apply(ppcheck_S, 1, mean)), xlab = 'AVG. S (estimated)', 
     main = '')
...
knitr::kable(
  sum_FReg[indx_pars, ], 
  digits = 2, 
  caption = 'Average, SD, confidence intervals and metrics of sampling diagnostics for the parameters estimating species richness and FReg of the birds assemblage'
)
```
...
knitr::kable(
  tribble(~ value,               ~estimation, 
        ...
        'AVG diff total-conditional (%)', 100-(mean(diff_crop_FReg)*100 / mean(conditional_crop_FReg$est))), digits = 2
)
...
```{r, fig.cap='Indirect and direct effect of crop size on the functional regularity (FReg) and species richness (S) of birds foraging on an S. terebinthifolia population. The effect on S (a) and FReg (b) denote indirect (i.e., X -> S -> FDiv) and direct effects, respectively (a and b). The dots indicate observed values, the solid line is the average estimation, the inner bands include 95% CI of the estimation, and the outer bands show 95% CI of the prediction. (c) Error bars and dots show 95% credibility intervals and average values of the causal effect of intervening factors to minimum and maximum values; the contrast shows the difference between such interventions. The total effect implies both direct and indirect effects of crop size on FReg, whereas the conditional effect denotes the direct effect of crop size on FReg while holding S on the minimum value.', fig.height= 15, fig.width=15}
