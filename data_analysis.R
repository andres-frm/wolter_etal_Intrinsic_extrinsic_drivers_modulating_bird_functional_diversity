...
apply(betas_S, 2, function(x) sd(x)/sqrt(length(dat$ind_id)))
apply(betas_S, 2, 
      function(x) {
...
apply(betas_FRic, 2, function(x) sd(x)/sqrt(length(dat$ind_id)))
apply(betas_FRic, 2, 
      function(x) {
        tibble(li = quantile(x, 0.025), 
               ls = quantile(x, 0.975))
...
diff_height_FRic <- mean(cont_height_FRic$est) - mean(conditional_height_FRic$est)


tribble(~ value,               ~estimation, 
        'P(total effect > 0)', mean(cont_height_FRic$est > 0), 
        'mu total effect', mean(cont_height_FRic$est), 
        'sd total effect', sd(cont_height_FRic$est)/sqrt(length(cont_height_FRic$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_height_FRic$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_height_FRic$porcentaje)/sqrt(length(cont_height_FRic$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_height_FRic$est > 0), 
        'mu conditional effect', mean(conditional_height_FRic$est), 
        'sd conditional effect', sd(conditional_height_FRic$est)/sqrt(length(conditional_height_FRic$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_height_FRic$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_height_FRic$porcentaje)/sqrt(length(conditional_height_FRic$porcentaje)),
...
diff_urban_FRic <- mean(cont_urban_FRic$est) - mean(conditional_urban_FRic$est)

tribble(~ value,               ~estimation, 
        'P(total effect < 0)', mean(cont_urban_FRic$est < 0), 
        'mu total effect', mean(cont_urban_FRic$est), 
        'sd total effect', sd(cont_urban_FRic$est)/sqrt(length(cont_urban_FRic$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_urban_FRic$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_urban_FRic$porcentaje)/sqrt(length(cont_urban_FRic$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_urban_FRic$est < 0), 
        'mu conditional effect', mean(conditional_urban_FRic$est), 
        'sd conditional effect', sd(conditional_urban_FRic$est)/sqrt(length(conditional_urban_FRic$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_urban_FRic$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_urban_FRic$porcentaje)/sqrt(length(conditional_urban_FRic$porcentaje)),
...
diff_forest_FRic <- mean(cont_forest_FRic$est) - mean(conditional_forest_FRic$est)

tribble(~ value,               ~estimation, 
        'P(total effect < 0)', mean(cont_forest_FRic$est < 0), 
        'mu total effect', mean(cont_forest_FRic$est), 
        'sd total effect', sd(cont_forest_FRic$est)/sqrt(length(cont_forest_FRic$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_forest_FRic$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_forest_FRic$porcentaje)/sqrt(length(cont_forest_FRic$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_forest_FRic$est < 0), 
        'mu conditional effect', mean(conditional_forest_FRic$est), 
        'sd conditional effect', sd(conditional_forest_FRic$est)/sqrt(length(conditional_forest_FRic$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_forest_FRic$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_forest_FRic$porcentaje)/sqrt(length(conditional_forest_FRic$porcentaje)),
        'P(total < conditional)', mean(cont_forest_FRic$est < conditional_forest_FRic$est), 
        'AVG diff total-conditional (%)', (mean(diff_forest_FRic)*100 / mean(cont_forest_FRic$est)-100))
...
apply(betas_FDiv, 2, function(x) sd(x)/sqrt(length(dat2$ind_id)))
apply(betas_FDiv, 2, 
      function(x) {
        tibble(li = quantile(x, 0.025), 
               ls = quantile(x, 0.975))
...
diff_height_FDiv <- mean(cont_height_FDiv$est) - mean(conditional_height_FDiv$est)


tribble(~ value,               ~estimation, 
        'P(total effect > 0)', mean(cont_height_FDiv$est > 0), 
        'mu total effect', mean(cont_height_FDiv$est), 
        'sd total effect', sd(cont_height_FDiv$est)/sqrt(length(cont_height_FDiv$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_height_FDiv$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_height_FDiv$porcentaje)/sqrt(length(cont_height_FDiv$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_height_FDiv$est > 0), 
        'mu conditional effect', mean(conditional_height_FDiv$est), 
        'sd conditional effect', sd(conditional_height_FDiv$est)/sqrt(length(conditional_height_FDiv$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_height_FDiv$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_height_FDiv$porcentaje)/sqrt(length(conditional_height_FDiv$porcentaje)),
...
diff_urban_FDiv <- mean(cont_urban_FDiv$est) - mean(conditional_urban_FDiv$est)

tribble(~ value,               ~estimation, 
        'P(total effect < 0)', mean(cont_urban_FDiv$est < 0), 
        'mu total effect', mean(cont_urban_FDiv$est), 
        'sd total effect', sd(cont_urban_FDiv$est)/sqrt(length(cont_urban_FDiv$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_urban_FDiv$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_urban_FDiv$porcentaje)/sqrt(length(cont_urban_FDiv$porcentaje)), 
        'P(conditional effect < 0)', mean(conditional_urban_FDiv$est < 0), 
        'mu conditional effect', mean(conditional_urban_FDiv$est), 
        'sd conditional effect', sd(conditional_urban_FDiv$est)/sqrt(length(conditional_urban_FDiv$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_urban_FDiv$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_urban_FDiv$porcentaje)/sqrt(length(conditional_urban_FDiv$porcentaje)),
...
diff_crop_FDiv <- mean(cont_crop_FDiv$est) - mean(conditional_crop_FDiv$est)

tribble(~ value,               ~estimation, 
        'P(total effect > 0)', mean(cont_crop_FDiv$est > 0), 
        'mu total effect', mean(cont_crop_FDiv$est), 
        'sd total effect', sd(cont_crop_FDiv$est)/sqrt(length(cont_crop_FDiv$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_crop_FDiv$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_crop_FDiv$porcentaje)/sqrt(length(cont_crop_FDiv$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_crop_FDiv$est > 0), 
        'mu conditional effect', mean(conditional_crop_FDiv$est), 
        'sd conditional effect', sd(conditional_crop_FDiv$est)/sqrt(length(conditional_crop_FDiv$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_crop_FDiv$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_crop_FDiv$porcentaje)/sqrt(length(conditional_crop_FDiv$porcentaje)),
...
apply(betas_S, 2, function(x) mean(x > 0))
apply(betas_S, 2, function(x) mean(x))
apply(betas_S, 2, function(x) sd(x)/sqrt(length(dat3$freg)))
apply(betas_S, 2, 
      function(x) {
        tibble(li = quantile(x, 0.025), 
               ls = quantile(x, 0.975))
...
apply(betas_FReg, 2, function(x) sd(x)/sqrt(length(dat3$freg)))
apply(betas_FReg, 2, 
      function(x) {
        tibble(li = quantile(x, 0.025), 
               ls = quantile(x, 0.975))
...
diff_crop_FReg <- mean(conditional_crop_FReg$est) - mean(cont_crop_FReg$est)

tribble(~ value,               ~estimation, 
        'P(total effect > 0)', mean(cont_crop_FReg$est > 0), 
        'mu total effect', mean(cont_crop_FReg$est), 
        'sd total effect', sd(cont_crop_FReg$est)/sqrt(length(cont_crop_FReg$est)), 
        'AVG change relative to maximum (total) (%)', mean(cont_crop_FReg$porcentaje), 
        'SD change relative to maximum (total) (%)', sd(cont_crop_FReg$porcentaje)/sqrt(length(cont_crop_FReg$porcentaje)), 
        'P(conditional effect > 0)', mean(conditional_crop_FReg$est > 0), 
        'mu conditional effect', mean(conditional_crop_FReg$est), 
        'sd conditional effect', sd(conditional_crop_FReg$est)/sqrt(length(conditional_crop_FReg$est)), 
        'AVG change relative to maximum (conditional) (%)', mean(conditional_crop_FReg$porcentaje), 
        'SD change relative to maximum (conditional) (%)', sd(conditional_crop_FReg$porcentaje)/sqrt(length(conditional_crop_FReg$porcentaje)),
